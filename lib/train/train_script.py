...
from .actors.cache_tracker import CACHETActor
...
from ..models.cache_tracker.cache_tracker import build_cachet
...
def run(settings):
    ...
    elif "cachet" in settings.script_name:
        net = build_cachet(cfg)
    ...
    elif "cachetcls" in settings.script_name:
        objective = {'cls': BCEWithLogitsLoss()}
        loss_weight = {'cls': 1.0}
        actor = STARKSTActor(net=net, objective=objective, loss_weight=loss_weight, settings=settings)
    elif "cachet" in settings.script_name:
    ...
    # train process
    if settings.project_path_prv!=None and settings.project_path_prv!='':
        trainer.train(cfg.TRAIN.EPOCH, load_latest=True, fail_safe=False, load_previous_ckpt=True)
    else:
