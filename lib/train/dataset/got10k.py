...
class Got10k(BaseVideoDataset):
    ...
    def __init__(self, root=None, image_loader=jpeg4py_loader, split=None, seq_ids=None, data_fraction=None):
        ...
        if None in self.seq_per_class:
            self.seq_per_class['None']=self.seq_per_class[None]
            del self.seq_per_class[None]
    ...
    def _read_target_visible(self, seq_path):
        ...
        if not os.path.exists(occlusion_file) or not not os.path.exists(cover_file):
            return None,None
    ...
    def get_sequence_info(self, seq_id):
        ...
        if visible is None or visible_ratio is None:
            visible = torch.ones(bbox.shape[0], dtype=torch.uint8)
            visible_ratio = torch.ones(bbox.shape[0], dtype=torch.float32)
