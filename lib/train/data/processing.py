...
class ErrorJitterProcessing(BaseProcessing):


    def __init__(self, search_area_factor, output_sz, center_jitter_factor, scale_jitter_factor,
                 error_center_jitter_factor=None, error_scale_jitter_factor=None,
                 mode='pair', settings=None, *args, **kwargs):
        """
        args:
            search_area_factor - The size of the search region  relative to the target size.
        ...
        self.center_jitter_factor = center_jitter_factor
        self.scale_jitter_factor = scale_jitter_factor
        self.error_center_jitter_factor = error_center_jitter_factor
        self.error_scale_jitter_factor = error_scale_jitter_factor
        self.mode = mode
        self.settings = settings
    ...
    def _get_error_jittered_box(self, box, mode):

        jittered_size = box[2:4] * torch.exp(torch.randn(2) * self.error_scale_jitter_factor[mode])
        max_offset = (jittered_size.prod().sqrt() * torch.tensor(self.error_center_jitter_factor[mode]).float())
        jittered_center = box[0:2] + 0.5 * box[2:4] + max_offset * (torch.rand(2) - 0.5)
    ...
    def __call__(self, data: TensorDict):
        ...
        for s in ['template', 'search']:
            ...
            # Add a uniform noise to the center pos
            jittered_anno = [self._get_jittered_box(a, s) for a in data[s + '_anno']]
            # apply error jitter for templates except gt
            if s=='template':
                error_jittered_anno = [data[s + '_anno'][0]]+[self._get_error_jittered_box(a, s) for a in data[s + '_anno'][1:]]
            ...
            if s=='search':
                ...
                crops, boxes, att_mask, mask_crops = prutils.jittered_center_crop(data[s + '_images'], error_jittered_anno,
                                                                                  error_jittered_anno,
