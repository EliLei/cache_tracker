...
class STARK_ST(BaseTracker):
    ...
    def initialize(self, image, info: dict):
        ...
        self.last_update = [0]*self.num_extra_template
        # save states
    ...
    def track(self, image, info: dict = None):
        ...
        #plt.figure('search');plt.imshow(x_patch_arr);
        ...
        for idx, update_i in enumerate(self.update_intervals):
            if self.frame_id % update_i == 0 and conf_score > 0.5:
                ...
                self.last_update[idx] = self.frame_id
    ...
    def saveimg(self, image, gt=False):
        ...
        if gt:
            ...
            cv2.putText(image_BGR, r'Template:',
                        (startx, starty + 1 * p), cv2.FONT_HERSHEY_SIMPLEX, fontsize, color=tcolor, thickness=thickness)
            cv2.putText(image_BGR, "-".join([f'f{t}' for t in self.last_update]),
